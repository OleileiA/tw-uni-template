<template>
  <view
    ...
    <view class="relative flex-none w-13d75 h-13d75 rounded-sm overflow-hidden">
      ...
      <u-icon v-if="playing" @click="clickPause" class="absolute inset-0 m-auto w-10 h-10"
              name="pause" size="30" color="#fff"></u-icon>
      <u-icon v-else @click="clickPlay" class="absolute inset-0 m-auto w-10 h-10"
              name="play-right-fill" size="30" color="#fff"></u-icon>
    </view>
    <view class="flex-auto ml-std">
      <slider class="m-0"
              ...
              block-color="#FF0000"
              activeColor="#FF0000"
              backgroundColor="rgba(255, 255, 255, 0.8)"></slider>
