<template>
  <view class="relative w-full h-70 box-border bg-fixed bg-no-repeat bg-cover"
        :style="{ backgroundImage: 'url(' + bg + ')' }">
    <view class="absolute inset-0 px-12 pt-6d25 pb-16d5 bg-black bg-opacity-50">
      <scroll-view class="mask w-full h-full text-xl text-white font-semibold"
                   scroll-y>
        <view v-for="item in lyric" :key="item"
              class="text-center leading-10">
          {{item}}
        </view>
      </scroll-view>
